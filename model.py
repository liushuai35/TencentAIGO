"""
该文件定义了推荐模型的核心组件和整体架构。

主要包含三个类：
1.  `FlashMultiHeadAttention`: 实现了多头注意力机制。如果PyTorch版本支持，会使用`scaled_dot_product_attention`（Flash Attention）以提高效率；否则，会回退到标准的自注意力实现。
2.  `PointWiseFeedForward`: 实现了逐点前馈网络，通常在Transformer块中跟在自注意力层之后，用于非线性变换。
3.  `BaselineModel`: 这是整个推荐模型的主体。它构建了一个基于Transformer的序列推荐模型。
    -   **初始化**:
        -   接收用户/物品数量、特征统计信息、设备等参数。
        -   创建各种Embedding层，包括物品、用户、稀疏特征、多模态特征等。
        -   构建Transformer层（`Encoder`），包含多头注意力和前馈网络。
    -   **特征处理 (`_process_features`)**:
        -   将不同类型的特征（稀疏、连续、数组、多模态）从输入batch中提取出来，并通过各自的Embedding层或线性变换层，转换成统一维度的向量。
        -   将所有特征向量融合（相加）成最终的物品或用户表示。
    -   **前向传播 (`forward`)**:
        -   处理序列特征、正样本特征和负样本特征，得到它们的Embedding。
        -   将序列Embedding输入Transformer编码器，得到上下文感知的序列表示。
        -   计算序列表示与正/负样本之间的点积得分，用于后续的损失计算。
    -   **推理 (`predict`)**:
        -   在推理阶段，处理输入的用户序列，通过Transformer编码器得到用户兴趣表示。
        -   计算用户兴趣表示与所有候选物品Embedding之间的得分。
        -   返回得分最高的top-k个物品作为推荐结果。
    -   **保存Embedding (`save_all_item_emb`)**:
        -   一个辅助方法，用于在训练后处理和保存所有物品的最终Embedding，以便于后续的在线推理或ANN检索。
"""
